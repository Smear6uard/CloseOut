...
- **Backend**: Convex (database, functions, file storage, HTTP actions)
- **Auth**: Clerk (JWT-based, synced to Convex via `useEnsureUser`)
- **Billing**: Polar (checkout, customer portal, webhooks)
- **AI**: OpenAI GPT-4o Vision (defect classification, before/after comparison)
...
| `RESEND_API_KEY` | Convex | Resend API key (optional, for email) |
| `VITE_APP_URL` | Client | App base URL (e.g. `http://localhost:5173`) |
| `OPENAI_API_KEY` | Convex | OpenAI API key (for AI features) |

**Client** vars use `VITE_` prefix and are available in the browser.
**Convex** vars are set in the Convex dashboard and available in backend functions.

## Clerk Setup

1. Create a Clerk application at [clerk.com](https://clerk.com)
2. In Clerk dashboard, go to **JWT Templates** and create a template named `convex`
3. Copy the Issuer URL — this is your `CLERK_JWT_ISSUER_DOMAIN`
4. Set the publishable key and secret key in your env

## Polar Setup

1. Create a Polar organization at [polar.sh](https://polar.sh)
2. Create two products: **Pro** and **Team** with desired pricing
...
5. Select subscription events: `subscription.created`, `subscription.updated`, `subscription.revoked`
...
Set all Convex env vars in the [Convex dashboard](https://dashboard.convex.dev).
